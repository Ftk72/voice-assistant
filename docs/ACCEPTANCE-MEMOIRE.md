...
Issus de la session de grilling du 2026-07-02 (14 questions), réalignés sur la
stack modulaire (ADR 0009) : l'injection et l'extraction mémoire sont pilotées
par le **Dialogue Forge** (REST direct vers memory-forge), le recall et l'oubli
sont des outils MCP — plus de Filter OpenWebUI (abandonné, ADR 0009). Vocabulaire :
voir CONTEXT.md § Mémoire.

## Scénario souvenir (le scénario « Léa »)

1. Lundi, en conversation, je dis : « ma fille Léa commence le judo mercredi ».
2. La conversation terminée (clôture → capture, ADR 0011), l'extraction différée peuple le graphe (entités Léa/judo, fait daté, provenance conversation).
...
- **100 % local** : injection/extraction directes par le Dialogue Forge, recall/oubli via MCP ; Neo4j, embedder et Memory Forge liés à 127.0.0.1.
