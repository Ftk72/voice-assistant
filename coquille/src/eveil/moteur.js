// ┌──────────────────────────────────────────────────────────────────────────┐
// │ CODE TIERS VENDORÉ — NE PAS RÉÉCRIRE, NE PAS TRADUIRE.                   │
// └──────────────────────────────────────────────────────────────────────────┘
//
// Origine : paquet npm `openwakeword-wasm-browser@0.1.1` (fichier
// `src/WakeWordEngine.js`), portage web d'openWakeWord par Diego Navarro
// (https://github.com/dnavarrom/openwakeword_wasm).
//
// Licence : MIT, déclarée par le `package.json` du paquet npm. ATTENTION : le
// dépôt GitHub ne porte AUCUN fichier LICENSE (`license: null` à l'API) — c'est
// la publication npm qui vaut concession. D'où le vendoring depuis le tarball
// npm et non depuis git : c'est la source dont la licence est explicite.
//
// Pourquoi vendoré plutôt qu'en dépendance (note de recherche 0009) : le
// portage n'a qu'un mainteneur, 8 commits, aucune release taguée. Internalisé,
// il ne fait que ~16 Ko de JS que nous pouvons maintenir nous-mêmes ; en
// dépendance vivante, il serait un risque d'approvisionnement. Zéro CDN, zéro
// réseau (ADR 0007) : audit du 2026-07-17, aucun littéral réseau dans ce code.
//
// Ce fichier est conservé VERBATIM pour rester resynchronisable avec l'amont.
// La règle « tout en français » du CLAUDE.md vise notre code, pas le code
// tiers : notre câblage est dans `../eveil.js`, en français.
//
// SEULE MODIFICATION apportée à l'original :
//   - l'import `from 'onnxruntime-web'` devient un chemin relatif vers le
//     runtime vendoré (la coquille n'a ni bundler ni resolver de modules npm).
//
